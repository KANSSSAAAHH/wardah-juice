...
<title>Kontak - Wardah Juice</title>
<style>
  :root{
    --green-900:#123317;
    --green-800:#164d20;
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  .kontak-leaf svg{width:28px; height:28px; color:var(--green-600); stroke:var(--green-600);}
  .kontak-hero h1{
    font-family:var(--font-display); font-weight:700; font-size:clamp(28px,3.6vw,40px);
    margin-bottom:12px;
  }
  .kontak-hero p{
    color:var(--ink-soft); font-size:15px; line-height:1.7; max-width:520px; margin:0 auto;
  }

  /* ===== KONTAK CONTENT ===== */
  .kontak-content{padding:44px 0 20px;}
  .kontak-grid{display:grid; grid-template-columns:1fr 1.15fr; gap:24px; align-items:start;}
  .info-card, .form-card{
  ...
  .info-item{display:flex; align-items:flex-start; gap:14px; margin-bottom:26px;}
  .info-item:last-child{margin-bottom:0;}
  .info-icon{
    width:40px; height:40px; border-radius:12px; flex-shrink:0;
    background:var(--green-100); display:flex; align-items:center; justify-content:center;
  }
  .info-icon svg{width:20px; height:20px; color:var(--green-600); stroke:var(--green-600);}
  .info-item h4{font-family:var(--font-display); font-weight:600; font-size:15px; margin-bottom:5px;}
  .info-item p, .info-item a{color:var(--ink-soft); font-size:14px; line-height:1.6;}
  .info-item a:hover{color:var(--green-700);}

  .form-field{margin-bottom:16px;}
  .form-field input, .form-field textarea{
    width:100%; border:1.5px solid var(--line); border-radius:var(--radius-md);
    padding:13px 16px; font-family:var(--font-body); font-size:14.5px; color:var(--ink);
    outline:none; transition:border-color .15s ease; resize:none;
  }
  .form-field input:focus, .form-field textarea:focus{border-color:var(--green-500);}
  .form-field textarea{min-height:96px;}
  .form-card .btn{width:100%; margin-top:4px;}

  /* ===== MAP ===== */
  .map-section{padding:20px 0 12px;}
  .map-frame{
...
<section class="kontak-hero">
  <div class="wrap">
    ...
    <h1>Kontak Kami</h1>
    <p>Kami siap melayani pesanan Anda setiap hari. Hubungi kami melalui informasi di bawah ini.</p>
  </div>
</section>
    ...
    <div class="kontak-grid">
      ...
      <div class="info-card">
        ...
        <div class="info-item">
          ...
          <div>
            <h4>Alamat</h4>
            <p>Perumahan kahuripan nirwana village blok D nomor 95</p>
          </div>
        </div>
        ...
        <div class="info-item">
          ...
          <div>
            <h4>WhatsApp</h4>
            <a href="https://wa.me/6283892606102" target="_blank">+6283892606102</a>
          </div>
        </div>
      ...
      <div class="form-card">
        ...
        <form id="kontakForm">
          ...
          <div class="form-field">
            <input type="tel" id="nomorWa" placeholder="Nomor WhatsApp" required>
          </div>
          <div class="form-field">
            <input type="text" id="produkDiminta" placeholder="Produk Yang Diminati">
          </div>
          <div class="form-field">
            <textarea id="pesanText" placeholder="Tuliskan Pesan atau pertanyaan Anda"></textarea>
          </div>
...
<section class="map-section">
  <div class="wrap">
    <div class="map-frame">
      <iframe
        src="https://www.google.com/maps?q=Perumahan+Kahuripan+Nirwana+Village+Blok+D+95,+Sidoarjo&output=embed"
        loading="lazy"
...
<script>
  ...
  // Kirim Pesan -> buka WhatsApp dengan isi form
  const kontakForm = document.getElementById('kontakForm');
  kontakForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const nama = document.getElementById('namaLengkap').value.trim();
    const wa = document.getElementById('nomorWa').value.trim();
    const produk = document.getElementById('produkDiminta').value.trim();
    const pesan = document.getElementById('pesanText').value.trim();

    let text = `Halo Wardah Juice, saya ingin bertanya:%0A`;
    text += `Nama: ${nama}%0A`;
    text += `No. WhatsApp: ${wa}%0A`;
    if(produk) text += `Produk yang diminati: ${produk}%0A`;
    if(pesan) text += `Pesan: ${pesan}`;
